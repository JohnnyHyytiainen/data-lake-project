...
`_is_de_relevant()` i producern - Jag diskuterade detta i teorin med LLM: filtrera tidigt. Det finns ett medvetet designbeslut här som är värd att förstå påpekade LLM. Bronze ska inte vara en soptipp av allt GitHub producerar - det ska vara rådata JAG valt att samla in. Det är skillnaden mellan att spara en hel sjö och att samla regnvatten i en tunna(tänk data lake som degrade'ar till ett data swamp)
